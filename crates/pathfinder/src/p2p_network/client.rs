...
use pathfinder_common::{
    ...
    ContractAddressSalt, Fee, SequencerAddress, StateCommitment, TransactionHash, TransactionNonce,
        ...
        match self {
            ...
            HybridClient::NonPropagating { p2p_client, .. } => match block {
                BlockId::Number(n) => {
                    ...
                    let header = conv::header::from_p2p(header);
        ...
        match self {
            ...
            HybridClient::NonPropagating { p2p_client, .. } => match block {
                BlockId::Hash(hash) => {
                    ...
                    let (block_hash, state_diff) = conv::state_diff::from_p2p(state_update);
