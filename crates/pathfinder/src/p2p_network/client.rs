...
pub mod conv {
    pub mod header {
        ...
        pub fn try_from_p2p(header: p2p_proto::common::BlockHeader) -> anyhow::Result<BlockHeader> {
            Ok(BlockHeader {
                ...
                class_commitment: ClassCommitment::new(header.class_commitment).ok_or(
                    anyhow::anyhow!("Out of range class commitment in block {}", header.number),
                )?,
    ...
    pub mod state_diff {
        ...
        pub fn try_from_p2p(
            su: BlockStateUpdateWithHash,
        ) -> anyhow::Result<(BlockHash, gw::StateDiff)> {
            Ok((
